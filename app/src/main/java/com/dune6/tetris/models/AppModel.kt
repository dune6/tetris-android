...
import com.dune6.tetris.constants.CellConstants
import com.dune6.tetris.constants.FieldConstants
import com.dune6.tetris.helper.array2dOfByte
import com.dune6.tetris.storage.AppPreferences
...
class AppModel {
    ...
    fun generateField(action: String) {
        if (isGameActive()) {
            ...
            when (action) {
                ...
                Motions.DOWN.name -> {
                    coordinate?.x = currentBlock?.position?.y?.minus(1)
                }
                Motions.LEFT.name -> {
    ...
    private fun shiftRows(nToRowId: Int) {
        if (nToRowId > 0) {
            for (j in nToRowId - 1 downTo 0) {
        ...
        for (j in 0 until field[0].size) setCellStatus(0, j, CellConstants.EMPTY.value)
    }
